...
public class HashBookImplementation implements Book {
	...
	private void addToWords(Words Word) throws OutOfMemoryError {
		...
		if (!filter.shouldFilter(Word.word) && Word.word.length() >= 2) {
			...
			for(int i=0;;i++){
				...
				if(words[tmpIndex]==null){
					...
					uniqueWordCount++;
					break;
				}else if(words[tmpIndex].word.equals(Word.word)){
	...
	private void reallocateArray(int newSize) throws OutOfMemoryError {
		Words[] newWordArray = new Words[newSize];
		int newTotalWordCount = 0;
		for (int i = 0; i < newSize && i < words.length; i++) {
			newWordArray[i] = words[i];
			newTotalWordCount += words[i].count;
		}
		words = newWordArray;
	...
	private void reallocate(int newSize) throws OutOfMemoryError {
		...
		reallocationCount++;
		Words[] oldWordList = words;
		this.words = new Words[(int) (newSize * (1.0 + LOAD_FACTOR))];

		uniqueWordCount = 0;
		collisionCount = 0;
		maxProbingSteps = 0;
		for (Words oldWord : oldWordList) {
			if (oldWord != null) {
				addToWords(oldWord);
			}
		}
